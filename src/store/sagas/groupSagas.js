...
function* createGroup({name, start, end}){
    try{
        const groups = yield axios.post(`${BASE_URL}/api/groups`, {name,start,end}, {headers: {'authorization': `Bearer ${localStorage.getItem('token')}`}}).then(res => res.data);
...
function* deleteActiveGroup({id}){
    try{
        yield axios.delete(`${BASE_URL}/api/groups/${id}`, {headers: {'authorization': `Bearer ${localStorage.getItem('token')}`}}).then(res => res.data);
...
function* updateActiveGroup({id, name, start, end}){
    try{
        yield axios.put(`${BASE_URL}/api/groups`, {id, name: name, start:start, end:end}, {headers: {'authorization': `Bearer ${localStorage.getItem('token')}`}}).then(res => res.data);
